...
const PROGRESS_MESSAGES: Record<OrderStatus, string> = {
...
function buildItemsSummary(items: Array<{ quantity: number; name: string }>): string {
  return items.map(({ quantity, name }) => `${quantity}x ${name}`).join(", ");
}

function resolvePickupLabel(timeSlotId: string): string {
  return TIME_SLOTS.find((s) => s.id === timeSlotId)?.pickupDisplay ?? timeSlotId;
}
...
export function OrderCard({ order, onCancel }: OrderCardProps) {
  ...
  const progressMsg = PROGRESS_MESSAGES[order.status];
  const cancellable = order.status === "PREPARING" && !!onCancel;
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        description="Saldo akan dikembalikan ke dompet Anda. Tindakan ini tidak dapat diurungkan."
